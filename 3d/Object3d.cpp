...
void Object3d::StaticInitialize(DirectXCommon* dxCommon)
{
	...
	//グラフィクスパイプラインの初期化
	base->GraphicsPipeline(dxCommon);

	Model::StaticInitialize(base->dxcommon->GetDevice());
}

void Object3d::Base::GraphicsPipeline(DirectXCommon* dxCommon)
{
...
void Object3d::Update()
{
	...
	UpdateWorldMatrix();
	// ビュープロジェクション行列
	const XMMATRIX& matViewProjection = base->camera->GetViewProjectionMatrix();
	// カメラ座標
	const XMFLOAT3& cameraPos = base->camera->GetEye();
...
void Object3d::UpdateWorldMatrix()
{
	...
	// ワールド行列の合成
	if (isBillboard && base->camera) {
		const XMMATRIX& matBillboard = base->camera->GetBillboardMatrix();
